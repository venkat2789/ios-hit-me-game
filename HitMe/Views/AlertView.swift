        ...
        VStack(spacing: 10) {
            ...
            Button(action: {
                withAnimation(){
                    ...
                    if(game.round == 5){
                        game.calculateFinalPoints(points: points)
                        endOfGameAlertDisplayed = true
                    } else {
                        game.startNewRound(points: points)
                    }
                }
            }) {
                AlertButtonText(text: "Continue")
            }
        }
                .frame(maxWidth: 250)
                .padding()
...
struct AlertView_Previews: PreviewProvider {
    ...
    static private var endOfGameAlertDisplayed = Binding.constant(false)
